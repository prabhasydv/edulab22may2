...
const CourseQuestions = () => {
    ...
    const toggleAnswer = (index) => {
        const el = document.getElementById(`${tab}-answer-${index}`);
                ...
                <div className="flex justify-center gap-6 mb-8 border-b border-gray-300 dark:border-gray-600">
                    {['beginner', 'advanced'].map((label) => (
                        <button
                            ...
                            className={`px-4 py-2 font-semibold ${tab === label
                    ...
                    {courseData[tab].length > 0 ? (
                        courseData[tab].map((question, i) => (
                            <div key={i} className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                                ...
                                <div
                                    id={`${tab}-answer-${i}`}
                                    className="mt-2 pl-2 text-sm text-gray-700 dark:text-gray-300 hidden"
                                >
                                    Sample answer for: <strong>{question}</strong>. (Replace with real answer.)
                                </div>
                            </div>
                        ))
                    ) : (
